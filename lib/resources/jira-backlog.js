...
class JiraBacklog extends Backlog {
  init(opts) {
    ...
    return super.init(opts)
      .then(specConfig => inquirer.prompt(questions)
        .then(({ url, projectKey }) => extend(specConfig, { url, projectKey })));
  }
  ...
  add() {
    ...
    const path = urljoin(this.spec.config.url, 'secure/CreateIssue!default.jspa');
  ...
  show(opts) {
    ...
    const projectPath = urljoin(this.spec.config.url, 'browse', this.spec.config.projectKey);
