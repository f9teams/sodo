...
class TravisCIBuild extends Build {
  init(opts) {
    ...
    return super.init(opts).then(specConfig =>
      inquirer
        .prompt(questions)
        .then(({ visibility, owner, repository, token }) =>
          extend(specConfig, {
            visibility,
            owner,
            repository,
  ...
  show() {
    const url =
      this.spec.config.visibility === 'public'
        ? 'https://travis-ci.org'
        : 'https://travis-ci.com';
    const path = urljoin(
      url,
      this.spec.config.owner,
      this.spec.config.repository,
    );
