...
const { keyBy, has, cloneDeep, pick, omit, isUndefined } = require('lodash');
...
function projectDir() {
  ...
  try {
    const pd = execSync(cmd, { stdio: ['ignore', 'pipe', 'ignore'] })
      .toString()
      .trim();
    return pd;
  } catch (e) {
    const ex = new NoProject(
      `Project not found. Current working directory is not a git repository. "${cmd}" failed with message: ${e.message}`,
    );
    debug(ex);
    throw ex;
  }
...
function mergeResources(projectConfig, privateConfig) {
  ...
  Object.entries(
    privateResources,
  ).forEach(([resourceType, privateResourceSpecs]) => {
    const labeledProjectResources = keyBy(
      projectResources[resourceType],
      'label',
    );
...
module.exports = {
  ...
  mock: () => {
    ...
    const chain = {
      with: (type, resource) => {
        module.exports.resources[type].push(resource);
        return chain;
      },
    };
    return chain;
